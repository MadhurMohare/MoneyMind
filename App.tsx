...
import { View, FinancialState, Transaction, FinancialGoal, Debt, User, ProcessedFile } from './types';
...
const App: React.FC = () => {
  ...
  const handleFileUpload = async (base64: string, mimeType: string, fileInfo?: { name: string, size: number }) => {
    if (fileInfo) {
      ...
      if (alreadyProcessed) {
        showNotification(`File "${fileInfo.name}" has already been processed. Skipping to prevent duplicates.`, 'warning');
        return;
      }
    }
    ...
    try {
      ...
      setFinancialState(prev => {
        ...
        if (newTransactions.length === 0 && newDebts.length === 0 && !result.monthlyIncome) {
          showNotification("No new unique data found in this document.", "info");
        } else {
          showNotification(`Successfully imported ${newTransactions.length} new transactions and ${newDebts.length} new debts.`, 'success');
        }
  ...
  const generatePlan = async () => {
    ...
    try {
      ...
      showNotification("AI Financial Plan has been updated!", "success");
    } catch (error) {
      console.error("Error generating plan:", error);
      showNotification("Failed to generate AI plan.", "error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const addDebt = (debt: Omit<Debt, 'id'>) => {
    ...
    showNotification("Debt registered successfully.", "success");
  };
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        {isLoading && (
          <div className="absolute inset-0 bg-white/60 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
            ...
            <p className="text-slate-600 font-medium animate-pulse text-center px-4">Analyzing your financial documents...</p>
          </div>
        )}

        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
          <h1 className="text-lg font-semibold text-slate-800">
            {currentView === View.DASHBOARD && 'Financial Overview'}
            {currentView === View.UPLOADS && 'Analyze Documents'}
            {currentView === View.TRANSACTIONS && 'Transaction History'}
            {currentView === View.GOALS && 'Goal Management'}
            {currentView === View.DEBTS && 'Debt Management'}
            {currentView === View.PLAN && 'AI Financial Strategy'}
          </h1>
          <div className="flex items-center gap-4">
            <button onClick={generatePlan} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
              <i className="fas fa-sparkles"></i> Refresh AI Plan
            </button>
          </div>
        </header>
